...
activities." Pervasive and Mobile Computing 38 (2017): 92-109.
...
Gain (IG). The aim is to find the segmentations that have the maximum
information gain for any number of segmentations.

Definitions:
 - A channel is one of the variables in a multivariate time series

Things to note:
 - The splits returned in the time series are taken to be after the kth
   element, not before
...
def TopDown(Multivar_TS, k, step, double=1):
    ...
    :param double: paramater passed to Clean_TS for the purpose of removing
                   correlation from the time series. Leaving it at 1 is
                   reccomended
    ...
    :return: (splits,InformationGain,knee)
             ...
             splits represents the positions of splits that are found to be
             optimal in the time series. These splits are after the position
             they index, for instance, if there is a 2 in the array, then
             {0,1,2} is one segment and {3,4,5...} is another. The order of
             this array is important, and is not sorted. The first element is
             the split that was found to have the highest information gain,
             and the second has the second heighest, etc.

             Information Gain is an numpy array of floats of size n. The ith
             element of the arary represents the information gain found for
             the first i segments.

             knee is a number <= n that is the knee point of the time series.
             Choosing a balance between number of segments (usually creating a
             larger amount of information gain) vs the size of the segments
             (too many segments can make them very small). The knee point of
             the information gain vs segments curve is returned to knee.
    """
    ...
    for i in range(k):
        ...
        # If we did not make any progress from the
        # information gain we already had
        if maxIG == IG_arr[i]:
            # We didn't get any information gain from this,
...
def DP_IGTS(Integ_TS, k, step, double):
    ...
    for i in range(0, Len_TS, step):
        for j in range(i+1, Len_TS, step):
            for t in range(0, Num_TS):
                TS_dist[t] = Integ_TS[t, j] - Integ_TS[t, i]
            prop = ((j-i)/Len_TS)
            cost[i:i+step:1, j:j+step:1, 0] = prop * SH_Entropy(TS_dist)
...
def IG_Cal_Incremental(IG_old, Integ_TS, pos_old, new_pos):
    """
     ...
     :param Integ_TS: The integral (cumulative sum) of the time series to
                      ...
                      Representes as a numpy array of shape
                      (number of series, time)
    ...
    for j in range(Num_TS):
        # This operation here is meant to get the sum of all the elements
        # between lower and higher, not inclusive of higher. If lower is the
        # very beginning of the time series (represented as lower = -1), then
        # the cumulative sum at higher is equal to the sum between the start
        # and higher
        if lower == -1:
...
def Clean_TS(O_Integ_TS, double):
    """
      ...
      double == 0, normalise all values in the series to be between 0 and 1000
                   (where 1000 is the max in the channel and 0 is the min)
      double == 1, Appends the reverse of the time series to remove positive
                   correlation
      double == 2, normalises so that 0 is still 0 but 1000 is the max of the
                   time
      series
    ...
    for i in range(Num_TS):
        ...
        if double == 1:
            ...
            if sumVal == 0:
                Integ_TS = np.vstack((Integ_TS, np.array(to_append)))
            else:
