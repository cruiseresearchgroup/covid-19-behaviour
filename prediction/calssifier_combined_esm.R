...
for (u in 1:length(user_id)) {
  ...
  load(paste(user_id[u],"_feature_set_complete_hourly.RData"))
  if(u==1)
  {
...
if(target_name=="role")
{
  complete_feature_set1<-complete_feature_set
  
  }else
  {
    complete_feature_set1<-complete_feature_set[c(complete_feature_set[,label_column]!=3),]
    complete_feature_set1<-complete_feature_set[c(complete_feature_set[,label_column]%in%c(1,5)),]
  }


excludecolumn<-c(1,2,36:40)
...
for (i  in 1:5) {
  ...
  save(accuracy_result,file=paste(target_name,"_acc_hourly.RData",sep=""))
  write.csv(accuracy_result,file=paste(target_name,"_acc_hourly.csv",sep=""))
}
#scales <- list(x=list(relation="free"), y=list(relation="free"))
#bwplot(results, scales=scales)
save(var_imp,file=paste(target_name,"_var_imp_hourly.RData",sep=""))
write.csv(var_imp,file=paste(target_name,"_var_imp_hourly.csv",sep=""))
