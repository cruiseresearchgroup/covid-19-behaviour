...
Response_data<-esm_data[,c("code","Date","hour","role")]
...
for (u in 1:length(user_id)) {
  ...
  response_per_user$Date[response_per_user$hour==-1]<-format(as.Date(response_per_user$Date)-days(1),"%Y-%m-%d",tz=Sys.timezone())
